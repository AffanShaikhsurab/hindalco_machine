...
mongoose.connect("mongodb+srv://affanpics:affanaffan@cluster0.jafgy.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0/devicesDB", {
...
app.post("/add-device", async (req, res) => {
    try {
        ...
        const qrCodeData = `http://localhost:3000/device/${newDevice._id}`;
...
app.get("/devices", async (req, res) => {
    try {
        const devices = await Device.find(); // Fetch all devices from MongoDB
        res.json(devices);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
