            ...
            {devices.map(device => (
                <div key={device._id} style={{ marginBottom: "20px", border: "1px solid black", padding: "10px" }}>
                    ...
                    {selectedDevice === device._id && (
                        <div>
                            <img src={`mongodb+srv://affanpics:affanaffan@cluster0.jafgy.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0/devices/qr/${device._id}`} alt="QR Code" width="150" />
                        </div>
                    )}
                </div>
            ))}
